...
/// This joint models a screw joint allowing two bodies to simulaneously translate and rotate
/// relative to one another along one axis with two degrees of freedom.
...
/// all times. The translation along and the rotation about z-axis of F  their rates
/// specify the state of the joint.
/// Zero (z, θ) corresponds to frames F and M being coincident and aligned.
...
class ScrewJoint final : public Joint<T> {
 public:
  ...
  ScrewJoint(const std::string& name, const Frame<T>& frame_on_parent,
              const Frame<T>& frame_on_child, Vector2<double> damping)
  ...
  /// modeled as `f₁ = -damping₁⋅v₁` i.e. opposing motion, with v₁
  /// the translation rate along the z-th axis for `this` joint (see
  /// get_translational_velocity()) and f₁ the force on child body B at Mo and
  /// expressed in F. That is, f_BMo_F = (f₁). The damping torque (in N⋅m)
  ...
  Vector2<double> damping() const { return damping_; }
  ...
  ///                  See class documentation for details.
  /// @returns a constant reference to `this` joint.
  const ScrewJoint<T>& set_pose(systems::Context<T>* context,
                                 const T& p_FoMo_F,
                                 const T& theta) const {
    get_mobilizer()->set_translation(context, p_FoMo_F);
    get_mobilizer()->set_angle(context, theta);
...
const T get_translational_velocity(
  ...
  void set_default_translation(const double& p_FoMo_F) {
    Vector2<double> state(p_FoMo_F,
                          this->default_positions()[1]);
    this->set_default_positions(state);
  }

  /// Gets the default angle for `this` joint.
  /// @retval theta The default angle of `this` joint.
  double get_default_rotation() const { return this->default_positions()[1]; }
  ...
  void set_default_rotation(double theta) {
    Vector2<double> state = this->default_positions();
    state[1] = theta;
    this->set_default_positions(state);
  }
  ...
  /// @param[in] p_FoMo_F The desired default position of the joint
  /// @param[in] theta The desired default angle of the joint
  void set_default_pose(const double& p_FoMo_F, const double& theta) {
    Vector2<double> state(p_FoMo_F, theta);
    this->set_default_positions(state);
  }
  ...
  void set_random_pose_distribution(
      const Vector2<symbolic::Expression>& theta) {
 ...
 private:
  ...
  /// For a ScrewJoint, we must always have `joint_dof < 2` since there are
  /// two degrees of freedom (num_velocities() == 2). `joint_tau` is the force
  /// applied along the z-axis of the parent  frame F if `joint_dof = 0`,
  /// or the torque about the z-axis of the parent frame F if `joint_dof = 1`.
  ...
  void DoAddInDamping(const systems::Context<T>& context,
    ...
    Eigen::VectorBlock<Eigen::Ref<VectorX<T>>> tau =
        get_mobilizer()->get_mutable_generalized_forces_from_array(
            &forces->mutable_generalized_forces());
    const T& v_translation = get_translational_velocity(context);
    const T& v_angular = get_angular_velocity(context);
    const Vector2<double> damping_coeff = damping();
    tau[0] -= damping_coeff[0] * v_translation;
    tau[1] -= damping_coeff[1] * v_angular;
  }
