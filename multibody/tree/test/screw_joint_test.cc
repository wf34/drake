...
TEST_F(ScrewJointTest, GetJointLimits) {
  ...
  EXPECT_EQ(joint_->acceleration_upper_limits(),
            Vector2d::Constant(kAccelerationUpperLimit));
  EXPECT_EQ(joint_->damping(), Vector2d::Constant(kDamping));
}
...
TEST_F(ScrewJointTest, RandomState) {
  ...
  mutable_joint_->set_random_pose_distribution(
      Vector2<symbolic::Expression>(uniform(generator) + 1.0, uniform(generator) + 3.0));
  tree().SetRandomState(*context_, &context_->get_mutable_state(), &generator);
  EXPECT_GE(mutable_joint_->get_translation(*context_), 1.0);
  EXPECT_LE(mutable_joint_->get_translation(*context_), 2.0);
